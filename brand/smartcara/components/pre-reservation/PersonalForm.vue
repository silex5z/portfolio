<template>
  <form @submit.prevent>
    <div class="info__group">
      ...
      <div
        :style="{ order: `${order[4]}` }"
        class="info__item info__color d--flex"
      >
        <div class="d--flex">
          <label class="info__label">희망 색상</label>
          <label
            :class="{ on: userModel.color === 'g' }"
            for="colorGrey"
            class="color__input"
          >
            <input
              id="colorGrey"
              v-model="userModel.color"
              type="radio"
              name="wishColor"
              value="g"
            />
...
export default {
  ...
  props: {
    ...
    phoneCorpAuthApi: {
      type: Object,
      default: () => {
        return { check: '/api/kmc/authenticate', save: '/api/kmc/save' }
      }
    },
