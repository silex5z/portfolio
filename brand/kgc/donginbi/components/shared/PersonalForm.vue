<template>
  <form @submit.prevent>
    <div class="info__group">
      ...
      <!-- 추가되는 항목 -->
      <slot v-for="info in additionalInfo" :name="info"></slot>
    </div>
    <!-- end:: info__group -->
    ...
    <div v-if="agreements.length >= 1" class="agree__group">
      ...
      >
        ...
        <a
          ...
          target="_blank"
        >
          <span class="a11y">{{ agree.txt }}</span> 자세히 보기
        </a>
        <button
...
export default {
  ...
  props: {
    ...
    phoneCorpAuthApi: {
      ...
      default: () => {
        return {
          check: '/api/experience-kit/kmc/authenticate',
          save: '/api/experience-kit/kmc/authenticate/sms'
        }
      }
    },
  ...
  methods: {
    ...
    sendKmcAuth() {
      ...
      this.$axios
        ...
        .catch((err) => {
          this.setLoading(false)
          if (err.response.data.detail === 'ALREADY_WIN')
            this.openPop({ popName: 'duplicate' })
