...
DrawingBoard.Board.defaultOpts = {
  controls: ['DrawingMode', 'Size', 'Navigation', 'Color'],
...
DrawingBoard.Control.DrawingMode = DrawingBoard.Control.extend({
    ...
    initialize: function() {
        ...
        $.each(["pencil", "filler", "eraser"], $.proxy(function(k, value) {
            if (this.opts[value]) {
    ...
    _dropdownTemplate: function() {
        ...
        $.each(this.opts.dropdownValues, function(i, size) {
            tpl += DrawingBoard.Utils.tpl(
                '<li data-size="{{size}}"><span style="width: {{size}}px; height: {{size}}px; border-radius: {{size}}px;"></span></li>', { size: size }
            );
        });
        tpl += '</ul></div>';
        return tpl;
